...
```{r}
location_plus_fully_combined <- left_join(Fully_combined_dataset_original,
...
```{r}
Match <- left_join(location_plus_fully_combined,
                   USGS_info_NY,
                   by = "county_fips_code",
                   multiple = "first" ) 
...
```{r}
  left_join(combined2011thru2022,
            )
...
matchedddd <- match(USGS_info_NY$county_fips_code,
      location_plus_fully_combined$county_fips_code)

print(matchedddd)



USGS_2011_2022 <- left_join(combined2011thru2022,
                            USGS_info_NY,
                            by = )
```
...
## Combine Location Plus and USGS
```{r}
full_location_plus_USGS <- left_join(location_plus_fully_combined,
                                          USGS_info_NY,
                                          by = "county_fips_code",
                                          unmatched = "drop")
```
...
## Add County Data
```{r}
fully_combined_dataset <- left_join(full_location_plus_USGS,
                                    combined2011thru2022,
                                    by = "county") 

                                    
#sum(is.na(fully_combined_dataset$percent_unemployed))
```
...
## Combine USGS and HUD Location Data
```{r}
location_data <- left_join(USGS_info_NY,
                           Hud_county_zip_crosswalk,
                           by = "county_fips_code",
                           unmatched = "drop")
...
## Combine location data and county level data
```{r}
full_county_data <- full_join(combined2011thru2022,
                              location_data,
                              by = "county")
  
```

## Combine Hospital data and county/location data
```{r}
fully_combined_dataset <- left_join(Fully_combined_dataset_original,
                                    full_county_data,
                                    by = c("year" ))
```
