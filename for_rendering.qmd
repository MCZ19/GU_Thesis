...
name_change <- imputed2 %>% mutate(bed_size_imputed = bed_size) %>%
  mutate(difference_in_discharges_imputed = difference_in_discharges) %>% mutate(net_charity_care_cost_ten_thousand_imputed = net_charity_care_cost_ten_thousand) %>% mutate(net_patient_revenue_hundred_thousand_imputed = net_patient_revenue_hundred_thousand) %>% select(ends_with("imputed"))

kmeans_evaluation <- name_change %>% !is.nan(difference_in_discharges_imputed)
...
#just looking at a regressions

test <- lm(difference_in_discharges_imputed ~ hospital_ownership_type, data = full_imputed2)

summary(test)
testglance <- glance(test)
# 2075 obs.

test2 <- lm(difference_in_discharges_imputed ~ hospital_ownership_type + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed, data = full_imputed2)  

summary(test2)
test2glance <- glance(test2)
# 2066 obs.

test3 <- plm(difference_in_discharges_imputed ~ hospital_ownership_type + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed,
    data = full_imputed2,
...
test4 <- plm(difference_in_discharges_imputed ~ hospital_ownership_type + year + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed,
                data = full_imputed2,
    index = c("hospital_name.y", "year"),
    model = "within",
    effect = "twoways")
