...
```{r}
num_for_gt <- numeric_data %>% select(net_patient_revenue_hundred_thousand,
                                            bed_size,
                                            net_charity_care_cost_ten_thousand,
                                            difference_in_discharges, 
                                            inpatient_occupancy, 
...
label = list(net_patient_revenue_hundred_thousand ~ "Net Patient Revenue in Hundreds of Thousands of Dollars",
             bed_size ~ "Number of Hospital Beds",
net_charity_care_cost_ten_thousand ~ "Net Charity Care Costs in Tens of Thousands of Dollars",
difference_in_discharges ~ "Expected vs. Observed PPCs",
inpatient_occupancy ~ "Percent of Occupied Hospital Beds",
medicare_payer_mix ~ "Medicare Payer Mix",
hospital_operating_labor_cost ~ "Hospital Operating Labor Cost",
medicaid_payer_mix_numeric ~ "Medicaid Payer Mix",
direct_patient_care_labor_hours_numeric ~ "Direct Patient Care Hours",
percent_children_in_poverty ~ "Percent of Children in Poverty in the County",
percent_unemployed ~ "Percent of Unemployed in the County",
percent_smokers ~ "Percent of Smokers in the County",
percent_excessive_drinking ~ "Percent of Excessive Drinkers in the County",
percent_obese ~ "Percent of Obesity in the County",
percent_uninsured ~ "Percent Uninsured in the County",
percent_female ~ "Percent Women in the County",
percent_65_and_over ~ "Percent of Population over 65 in the County",
percent_not_proficient_in_english ~ "Percent non-English Speakers in the County",
percent_hispanic ~ "Percent of Population that is Hispanic",
percent_african_american ~ "Percent of Population that is African American",
household_income ~ "Average Household Income by County",
mentally_unhealthy_days ~ "Mentally Unhealthy Days",
population_y ~ "Total County Population",
percent_rural ~ "Percent of County Considered Rural",
...
rec2 <- numeric_te2 %>% recipe(net_patient_revenue_hundred_thousand ~ bed_size +
                                            net_charity_care_cost_ten_thousand +
                                            difference_in_discharges + 
                                            inpatient_occupancy + 
                                            operating_expenses +
...
test4 <- plm(difference_in_discharges_imputed ~ removed_omits$`Non-Profit_imputed` + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed + inpatient_occupancy_imputed + direct_patient_care_labor_hours_numeric_imputed +
              percent_children_in_poverty_imputed +
              percent_unemployed_imputed +
              percent_smokers_imputed +
              percent_excessive_drinking_imputed +
...
test5 <- plm(difference_in_discharges_imputed ~ removed_omits$`Non-Profit_imputed` + year + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed + inpatient_occupancy_imputed +
              direct_patient_care_labor_hours_numeric_imputed+
              percent_children_in_poverty_imputed +
              percent_unemployed_imputed +
              percent_smokers_imputed +
...
Complete_imputed_table <- stargazer(test,
          ...
          title = "KNN Imputed Regression Models", 
                                               ...
                                               single.row = TRUE,
                                               digits = 3,
                                               covariate.labels = c("Non-Profit Hospitals", "Number of Beds",
                                                                    ...
                                                                    "Total Population",
                                                                    "Percent Rural"), 
                                               column.labels = c("Simple Linear", "Linear with Hospital Controls", "Linear with Hospital and County Controls", "Hospital Fixed Effects", "Two Way Fixed Effects"),
...
ggplot(data = Fully_combined_dataset,
  ...
  xlab("Observed vs. Expected PPCs") +
  ylab("Density") +
