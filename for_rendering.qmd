...
View(numeric_data %>% group_by(hospital_name.y) %>% 
       select(year, 
              ...
              hospital_ownership_type, hospital_name.y))
...
test2 <- lm(difference_in_discharges_imputed ~ hospital_ownership_type + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed, data = removed_omits)  

summary(test2)
testglance2 <- glance(test2)
# 2066 obs.

test3 <- plm(difference_in_discharges_imputed ~ hospital_ownership_type + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed,
    data = removed_omits,
...
test4 <- plm(difference_in_discharges_imputed ~ hospital_ownership_type + year + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed,
                data = removed_omits,
    index = c("hospital_name.y", "year"),
    model = "within",
    effect = "twoways")
