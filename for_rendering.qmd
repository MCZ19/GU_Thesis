...
```{r}
kmean_evaluation <- numeric_data %>% select(net_patient_revenue_hundred_thousand, bed_size,
                                            net_charity_care_cost_ten_thousand, difference_in_discharges, 
                                            inpatient_occupancy, 
                                            operating_expenses,
                                            fund_balance,
                                            medicare_payer_mix,
...
test2 <- lm(difference_in_discharges_imputed ~ hospital_ownership_type + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed + inpatient_occupancy_imputed, data = removed_omits)  

summary(test2)
testglance2 <- glance(test2)
# 2066 obs.

test3 <- plm(difference_in_discharges_imputed ~ hospital_ownership_type + hospital_name.y + bed_size_imputed + net_patient_revenue_hundred_thousand_imputed + net_charity_care_cost_ten_thousand_imputed + medicare_payer_mix_imputed + medicaid_payer_mix_numeric_imputed + inpatient_occupancy_imputed,
    data = removed_omits,
