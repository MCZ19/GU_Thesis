...
View(numeric_data %>% group_by(hospital_name.y) %>% select(year, bed_size, net_charity_care_cost_ten_thousand,
...
Compare_lm_table <- stargazer(m1,
          m2, m3, m4,
          #fixed_effects, 
          type = "text",
          title = "Simple Linear Regression vs. Linear Regression with Controls", 
                                               report = "vcs*",
                                               single.row = TRUE,
                                               digits = 3,
                                               omit.stat = c("f","ser","rsq"),
